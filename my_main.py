...
while True:
    ...
    all_binary = r_binary & g_binary & b_binary

    cv.imshow('all_binary', all_binary)
    
    matrix = cv.getPerspectiveTransform(arc, dst)
    warp = cv.warpPerspective(all_binary, matrix, (img_size[0], img_size[1]), flags=cv.INTER_LINEAR)
