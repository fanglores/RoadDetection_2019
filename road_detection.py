...
while (cv2.waitKey(100) != 27):
    ...
    for window in range(nwindows):
        ...
        if len(good_left_inds) > 50:
            XCenterLeftWindow = np.int(np.mean(WhitePixelIndX[left_lane_inds]))
        if len(good_right_inds) > 50:
            XCenterRightWindow = np.int(np.mean(WhitePixelIndX[right_lane_inds]))
